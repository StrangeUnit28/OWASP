...
with open('users.txt', 'w') as file:
    for user_id in range(1, 6):
        response = requests.get(f"{url}{user_id}/", cookies=cookies)
        if response.status_code == 200:
            file.write(f"Dados para users/{user_id}:\n")
            file.write(str(response.json()) + "\n\n") 
        else:
            file.write(f"Erro na requisição para users/{user_id}. Status: {response.status_code}\n\n")
print("Dados das requisições foram salvos em 'users.txt'.")
