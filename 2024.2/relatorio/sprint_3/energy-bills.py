...
url = "http://localhost:8000/api/energy-bills/plot-graph/"
...
with open('energy-bills.txt', 'w') as file:
    for consumer_unit_id in range(1, 18):
        params = {"consumer_unit_id": consumer_unit_id}
        response = requests.get(url, cookies=cookies, params=params)
        if response.status_code == 200:
            file.write(f"Dados para consumer_unit_id={consumer_unit_id}:\n")
            file.write(str(response.json()) + "\n\n")
        else:
            file.write(f"Erro na requisição para consumer_unit_id={consumer_unit_id}. Status: {response.status_code}\n\n")

print("Dados das requisições foram salvos em 'energy-bills.txt'.")
